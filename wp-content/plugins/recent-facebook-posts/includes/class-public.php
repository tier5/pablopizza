...
?>
		...
		<div class="row recent-facebook-posts rfbp rfbp-container rfbp-<?php echo esc_attr( $atts['origin'] ); ?>">
			<?php
		...
		if ( $posts && ! empty( $posts ) ) {
			...
			foreach ( $posts as $p ) {
				...
				if ( $opts['img_size'] !== 'dont_show' && ! empty( $p['image'] ) ) {
				    $post_classes[] = 'rfbp-post-with-media col-md-6 col-sm-6 single-pic';
				}				
